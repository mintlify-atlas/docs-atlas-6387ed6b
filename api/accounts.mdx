...
description: 'Manage Claude accounts for load balancing and pooling'
---

## Overview

The Account Management API allows you to manage Claude accounts used by Codex-LB for load balancing. You can import accounts, list them with usage statistics, pause/reactivate accounts, and view usage trends.
...
<ParamField path="POST /api/accounts/import" type="endpoint">
  Import a Claude account using an auth.json file from the official Claude CLI.
</ParamField>

### Request

<ParamField body="auth_json" type="file" required>
  The auth.json file containing Claude session tokens. Must be uploaded as `multipart/form-data`.
</ParamField>
